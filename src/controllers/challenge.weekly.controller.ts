...
export class DateChallengeController extends Controller{
  ...
  @Tags('date challenge controller')
  ...
  public async handleNewWeeklyChallenge(
    ...
    const data: WeeklyChallengeCreation = bodyToWeeklyCreation(body, req.user.id);
  ...
  public async handleGetWeeklyChallenge(
    ...
    const result: ResponseFromChallenge = await serviceGetWeeklyChallenge(BigInt(challengeId))
    .then(r => {
      return r;
    })
    .catch(err => {
