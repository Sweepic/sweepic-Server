...
export class ChallengeController extends Controller{
  ...
  public async handleAcceptChallenge(
    ...
    const result: ResponseFromChallenge = await serviceAcceptChallenge(BigInt(acceptId))
    .then(r => {
      return r;
    })
    .catch(err => {
  ...
  public async handleCompleteChallenge(
    ...
    const result: ResponseFromChallenge = await serviceCompleteChallenge(BigInt(completeId))
    .then(r => {
      return r;
    })
    .catch(err => {
  ...
  public async naverController(
    ...
    if(!hashedLocation){
      throw new DataValidationError({reason: 'query문이 비었습니다. hashedLocation을 입력하세요.'});
    }
