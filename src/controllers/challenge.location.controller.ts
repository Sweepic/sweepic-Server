...
export class LocationController extends Controller {
  ...
  public async handleNewLocationChallenge(
    ...
    const data: LocationChallengeCreation = bodyToLocationCreation(
      body,
      req.user.id,
    );
  ...
  public async handleGetLocationChallenge(
    ...
    const result: ResponseFromLocationChallenge =
      await serviceGetLocationChallenge(BigInt(challengeId))
        .then(r => {
          return r;
        })
        .catch(err => {
